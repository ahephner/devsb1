...
export default class OrnAppRatePrice extends LightningElement {
    @track data; 
    @api areaSize;
    appTotalPrice;  
    loaded = true; 
...
//Pricing 
    ...
    newMargin(m){
     ...
     newQty(e){
        window.clearTimeout(this.delay)
             ...
             // eslint-disable-next-line @lwc/lwc/no-async-operation
             this.delay = setTimeout(()=>{
                     this.data[index].Units_Required__c = Number(e.detail.value);
                     if(Number(e.detail.value) > 0){
                        ...
                        //  this.prodCostG = costs.perThousand;
                        //  this.prodCost100 = costs.perAcre;
                        //  this.prodAreaCost = this.areaAcres * this.costPerAcre;
    ...
    get unitArea(){
     return [
         {label:'OZ/100 Gal', value:'OZ/100 Gal'},
         {label:'LB/100 Gal', value:'LB/100 Gal'}
     ];
 }
//handle note 
 prodNote(e){
...
//Update Pricing and Fertility info displayed to per M or per Acre
 ...
 hiMouse(e){
     ...
     this.prodCost100 = ornPerProduct(index.Unit_Price__c, index.Product_Size__c, index.Rate2__c).perGal.per100;
